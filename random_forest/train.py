...
cv_num = 3
cross = cross_val_score(rf, train_df.drop(260, axis=1), train_df[260], cv=cv_num)
for i in range(cv_num):
    print("Valid: {:.4f}".format(cross[i]))
rf.fit(train_df.drop(260, axis=1), train_df[260])
print("Training Acc: {:.4f}".format(rf.score(train_df.drop(260, axis=1), train_df[260])))
